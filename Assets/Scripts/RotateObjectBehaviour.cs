...
public class RotateObjectBehaviour : MonoBehaviour
{
    ...
    [SerializeField] protected float m_Amplitude;

    [SerializeField] protected float m_Speed;

    [SerializeField] protected bool m_UseHorizontalBounce;
    ...
    {
        float tempAmplitude =
            m_StartPosAxis + m_Amplitude * Mathf.Sin(m_Speed * Time.time);

        if (!m_UseHorizontalBounce)
        {
            transform.position = new Vector3(transform.position.x, tempAmplitude,
